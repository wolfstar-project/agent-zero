...
Creates `.agent-zero.yml` with the safe defaults (`mode: observe`, autofix disabled). See [Repository policy](/guide/configuration).
...
`zero --version` prints the version injected at build time by [`@redstardev/unplugin-version-injector`](https://www.npmjs.com/package/@redstardev/unplugin-version-injector) into `@agent-zero/shared`; the CLI displays the same version in its header.
