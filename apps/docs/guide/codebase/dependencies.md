...
Agent Zero uses [aube](https://aube.jdx.dev) as its package manager, pinned through the `packageManager` field in the root `package.json`. aube reads and writes the existing `pnpm-lock.yaml` and `pnpm-workspace.yaml` in place, so the lockfile stays in pnpm's v9 format for anyone who still runs pnpm.
