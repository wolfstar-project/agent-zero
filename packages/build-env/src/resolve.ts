...
import { shortenCommit } from './build-info.js';
import {
 ...
 *
 * Whatever is still unresolved is published as a sentinel rather than guessed at, so the run-time
 * pass (`runtimeBuildInfo`) can tell a field the build genuinely resolved from one it did not.
 */
export async function resolveBuildInfo(options: ResolveBuildInfoOptions): Promise<BuildInfo> {
  ...
  const branch = metadata.branch ?? git.branch ?? unknownRevision;
  const commit = metadata.commit ?? git.commit ?? unknownRevision;
  const env =
