...
Nuxt dashboard ───> operational interface ──> auth adapter ──> database package ──> Postgres
```
...
- `auth` holds authentication policy and the Better Auth instance and reads the store through `database`; `apps/auth-server` is the entry-point adapter that exposes it over HTTP. None of the three may depend on the runtime.
...
Authentication follows the same adapter rule. Better Auth runs in `apps/auth-server`, a standalone Hono process that mounts the handler at `/api/auth/*` and is the only process in the repository that opens a database connection. The dashboard consumes it as a client through `@onmax/nuxt-better-auth` in `clientOnly` mode, which drops the local `/api/auth/**` handlers, the server auth config, and the signing secret. `packages/auth` holds the policy and the instance factory so that the contract is expressible without an HTTP server; its `./config` subpath is free of database dependencies so the dashboard can read feature flags without bundling one.
